...
class BiLSTM_CRFs(object):
    def __init__(self, configs, logger, dataManager):
        ...
        self.output_test_file = Path(configs.predict_dir).joinpath(timestamp+'.csv').as_posix()
        self.is_output_sentence_entity = configs.is_output_sentence_entity
        self.output_sentence_entity_file = Path(configs.predict_dir).joinpath(timestamp + '_' + configs.output_sentence_entity_file).as_posix()
...
# ================= Session run ===============
        ...
        path_list = [self.logdir, self.checkpoints_dir]
...
# ========== eager execution problem in tf2 =============
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if np.array(dev_f1_avg).mean() > self.best_f1_val:
                ...
                best_at_epoch = epoch
                saver.save(self.sess, Path(self.checkpoints_dir).joinpath(self.checkpoint_name).as_posix(), global_step=self.global_step)
    ...
    def train_on_batch(self, ):
        # extract the matched wrong prediction with sens
        df_corr_part = self.dataManager.match_corr_diff()
        _, X_train, y_train, X_val, y_val = self.dataManager.getCorrSet(df_corr_part)
        ...
        # load saved model A
        saver = self.soft_load()

        # retrain and save loss
        tf.compat.v1.summary.merge_all()
        train_writer = tf.compat.v1.summary.FileWriter(PurePosixPath(self.logdir)/"retraining_loss",self.sess.graph)
        dev_writer = tf.compat.v1.summary.FileWriter(PurePosixPath(self.logdir)/"validating_loss",self.sess.graph)
    ...
    def test(self):
        ids, X_test, y_test_psyduo_label, X_test_str = self.dataManager.getTestingSet()
        ...
        for i in range(num_iterations):
            ...
            hash_ids_batch = ids[i * self.batch_size: (i + 1) * self.batch_size]
            ...
            if i == num_iterations - 1 and len(X_test_batch) < self.batch_size:
                ...
                y_test_psyduo_label_batch += [[self.dataManager.label2id['O'] for j in range(self.max_time_steps)] for i
                                              in range(gap)]
                # hash_ids_batch += [[str(np.nan) for j in range(self.max_time_steps)] for i in
                #                      range(gap)]
        ...
        def save_test_out(tokens:list, labels:list, hash_ids:list) -> None:
            # transform format
            newtokens, newlabels, newids = [], [], []
            for to, la, hash_id in zip(tokens, labels, ids):
                newtokens.extend(to)
                newtokens.append("")
                newlabels.extend(la)
                newlabels.append("")
                newids.extend(hash_id)
                newids.append("")
            # save
            save_csv_(pd.DataFrame({"id": newids, "token": newtokens, "label": newlabels}), self.output_test_file, ["id", "token", "label"],
                      delimiter=self.configs.delimiter)

        save_test_out(tokens, labels, hash_ids)
