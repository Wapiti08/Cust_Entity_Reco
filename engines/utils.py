...
def extractEntity(x, y, dataManager):
    ...
    elif label_scheme == 'BILUO':
        if label_level == 1:
            reg_str = r'([0-9][0-9][0-9]B' + r' )([0-9][0-9][0-9]I' + r' )*([0-9][0-9][0-9]L' + r' )|([0-9][0-9][0-9]U' + r' )'

        elif label_level == 2:
            tag_bodys = ["(" + tag + ")" for tag in dataManager.suffix]
            tag_str = "(" + ('|'.join(tag_bodys)) + ")"
            reg_str = r'([0-9][0-9][0-9]B'+label_hyphen + tag_str + r' )([0-9][0-9][0-9]I'+label_hyphen + tag_str + r' )*([0-9][0-9][0-9]L'+label_hyphen + tag_str + r' )|([0-9][0-9][0-9]U'+label_hyphen + tag_str + r' )'
...
def read_csv_(file_name, names, delimiter='t'):
    if delimiter=='t':
        ...
        return pd.read_csv(file_name, sep = ",", quoting = csv.QUOTE_NONE, engine='python', skip_blank_lines=False, header = None, names=names)
    else:
        sep = delimiter 

    return pd.read_csv(file_name, sep=sep, quoting=csv.QUOTE_NONE, engine='python', skip_blank_lines=False, header=None,
                       names=names)
